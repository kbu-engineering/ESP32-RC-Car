...
void processGamepad(ControllerPtr ctl) {
  ...
  //== Switch R2 button = 0x0080 ==//
  if (ctl->buttons() == 0x0080 && pos == 0) {
    servoCycleUp();
    delay(100);
  }
 
   //== Switch L2 button = 0x0040 ==//
  if (ctl->buttons() == 0x0040 && pos == 180) {
    servoCycleDown();
    delay(100);
  }
  ...
  if (ctl->axisY() <= -25) {
    ...
    int motorSpeed = map(ctl->axisY(), -25, -510, 70, 255);
  ...
  if (ctl->axisRX() <= -25) {
    // map joystick values to motor speed
    int motorSpeed = map(ctl->axisRX(), -25, -510, 70, 255);
    // turn robot left - move right motor forward, left motor backwards
    digitalWrite(IN1pin, LOW);
    digitalWrite(IN2pin, HIGH);
    analogWrite(ENApin, motorSpeed);
    digitalWrite(IN3pin, HIGH);
...
// servoCycle() function rotates servo 180 degrees
void servoCycleUp() {
    if(pos < 170){
